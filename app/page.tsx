...
export default function Home() {
  ...
  return (
    <main className="relative h-[100dvh] w-full flex flex-col justify-between overflow-hidden select-none">
      ...
      {/* Center Stage: Cinematic Dashboard for Desktop & Mobile */}
      <div className="relative z-10 flex-1 flex flex-col justify-center items-center px-3 sm:px-6 py-2 sm:py-4 overflow-y-auto scrollbar-none w-full max-w-7xl 2xl:max-w-[1600px] mx-auto pointer-events-auto">
        <div className="w-full flex flex-col items-center justify-center gap-3 sm:gap-4 my-auto animate-in fade-in zoom-in-95 duration-500">
          {/* Spotlight Story Card */}
          <TonightsPick
            story={currentStory || tonightsPick || stories[0]}
            currentPlayingStory={currentStory}
            isPlaying={isPlaying}
            isBookmarked={bookmarks.includes((currentStory || tonightsPick || stories[0]).id)}
            onPlay={handleSelectStory}
            onToggleBookmark={handleToggleBookmark}
            onOpenDrawer={() => setIsDrawerOpen(true)}
          />

          {/* Desktop & Tablet Genre Quick Shelf Carousel */}
          <div className="hidden md:block w-full max-w-5xl">
            <StoryShelf
              ...
              onPlayStory={handleSelectStory}
              onToggleBookmark={handleToggleBookmark}
              onOpenAllInDrawer={() => setIsDrawerOpen(true)}
            />
          </div>
        </div>
      </div>
